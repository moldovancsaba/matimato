...
- The first turn has no blob because no row/column constraint exists yet; every unclaimed card is playable but receives no track styling. Blob timing is centralized through the same settle duration used to lock input, and polling same-version game state must not restart or cancel that timer.
