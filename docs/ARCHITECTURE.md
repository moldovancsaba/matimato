...
- Board animation uses CSS-only progressive reveal groups from `-9` through `+9`, a filled row/column selection ribbon exactly behind the playable track, and a selected-cell compression fade. Claimed cells visually disappear like removed tiles, never show as empty buttons, and are never marked with `x`; playable state is indicated by the ribbon, not button strokes.
