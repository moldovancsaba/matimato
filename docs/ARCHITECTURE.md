...
- Move responses may return ordered public `animationFrames` whenever one request produces one or more committed move states. The client must play every frame through the same board renderer and blob controller regardless of mode, player type, or move source.
...
- Result view state is derived from the public game DTO and exposes outcome copy, final scores, rematch setup, home navigation, share text, and match-finished analytics without adding persistence. Terminal outcomes drive decorative result motion: victory uses a short burst/aura, defeat uses a subdued ember/drop animation, and reduced-motion users receive a static state.
...
- Quests, Ranks, History, and Profile use the same top-aligned feature screen rhythm: kicker, large title, controls, content. Large viewport filler must never push titles to the bottom of the panel. Profile keeps its avatar on the right so the left title edge aligns with the other feature pages.
...
- Board animation uses CSS-only progressive reveal groups from `-9` through `+9`, plus a single signature selection blob controlled by an explicit frontend action controller. The board renderer is mode-agnostic: SOLO, BATTLE, human moves, AI moves, local responses, and polled remote responses all enter the same ordered frame queue. For each accepted move frame, the client measures the rendered board in pixels, computes the previous track, selected cell, and next track rectangles, then uses Motion DOM animation promises to run `collapse -> commit visual board removal -> grow -> unlock`. The first move has no prior ribbon, so the blob starts at the selected cell position before growing into the first constrained track. Row, column, and cell blob postures are explicit visual states, with column ribbons measured as real full-height rectangles rather than inferred CSS states. Input stays locked until the actual grow animation finishes, so the next actor can only choose after the blob reaches their playable track; non-match, unconstrained, and terminal states explicitly cancel the controller so the board cannot become permanently inactive. Claimed cells visually disappear like removed tiles, never show as empty buttons, and are never marked with `x`; playable state is indicated by the blob, not button strokes.
- Blob geometry is measured from actual rendered tile DOMRects first, not from CSS formulas. The controller keeps refs for all 81 tiles, derives selected-cell rectangles from `getBoundingClientRect()`, derives full row/column tracks from the measured tile axis plus board padding, and falls back to a two-dimensional board model only if a tile ref is temporarily unavailable. CSS keyword gaps such as `normal` are ignored safely in fallback mode. This keeps vertical and horizontal ribbons visible and aligned across mobile browsers, dynamic viewport changes, and responsive board sizing.
- Blob animation is action-sequenced, not timer-sequenced: collapse finishes before the card is removed, board state commits after the collapse, grow finishes before input unlocks, and the next actor cannot move until that promise resolves. Reduced-motion users skip the Motion movement while preserving the same state ordering and input lock lifecycle.
- The blob controller is version-gated. If polling returns the same game version already rendered, the client does not call `setGame`, does not restart animation, and does not mutate the blob except to initialize a missing ready track.
- Blob rendering is controller-first only while an action is actively collapsing or growing. Idle ready-track rendering is always derived from the current `game.constraintView`, never from a stored measured rectangle, so polling, viewport changes, or completed animation frames cannot leave the ribbon on an old row/column while the HUD announces a new playable track.
