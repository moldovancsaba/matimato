...
- Blob rendering is controller-first. If an animation frame owns a blob rectangle, the renderer must mount that blob even when the currently committed visual game has no `constraintView` yet. `game.constraintView` is only the fallback source for an idle ready-track blob.
