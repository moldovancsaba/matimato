...
- SOLO move responses return ordered public animation frames when the server also applies an AI reply. The client must play each frame sequentially so the human move grows the vertical column first, then the AI move collapses that column and grows the next horizontal row.
...
- Board animation uses CSS-only progressive reveal groups from `-9` through `+9`, plus a single signature selection blob controlled by an explicit frontend action controller. For each accepted move frame, the client measures the rendered board in pixels, computes the previous track, selected cell, and next track rectangles, then uses WAAPI `animation.finished` promises to run `collapse -> commit visual board removal -> grow -> unlock`. SOLO AI turns are not collapsed into one final-state animation; the API returns ordered animation frames and the client plays them one by one. The first move has no prior ribbon, so the blob starts at the selected cell position before growing into the first constrained track. Row, column, and cell blob postures are explicit visual states, with column ribbons measured as real full-height rectangles rather than inferred CSS states. Input stays locked until the actual grow animation finishes, so the opponent can only choose after the blob reaches their playable track; non-match, unconstrained, and terminal states explicitly cancel the controller so the board cannot become permanently inactive. Claimed cells visually disappear like removed tiles, never show as empty buttons, and are never marked with `x`; playable state is indicated by the blob, not button strokes.
