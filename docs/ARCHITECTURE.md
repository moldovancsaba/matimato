...
- The active match board is a dedicated SVG game renderer, not an HTML/CSS layout. `MatimatoBoard` owns a fixed `900x900` game world, deterministic tile coordinates, deterministic row/column track rectangles, the signature blob, keyboard/pointer cell input, and the action animation state machine. Responsive behavior scales the entire SVG game world as one unit, like a browser game canvas; CSS never computes ribbon geometry.
- Board animation is mode-agnostic: SOLO, BATTLE, human moves, AI moves, local responses, and polled remote responses all enter the same ordered frame queue. For each accepted move frame, `MatimatoBoard` computes the previous track, selected cell, and next track from the fixed board coordinate engine, then runs a requestAnimationFrame game loop that writes SVG `x`, `y`, `width`, and `height` attributes directly through `collapse -> local tile removal -> grow -> commit game state -> unlock`. The first move has no prior ribbon, so the blob starts at the selected cell position before growing into the first constrained track. During animation, the selected tile is hidden inside the board renderer only; the parent game DTO, HUD, scores, legal cells, and turn copy are committed after the blob reaches the next playable track. Input stays locked until that commit completes, so the next actor can only choose after the blob and HUD agree. Claimed cells visually disappear like removed tiles, never show as empty buttons, and are never marked with `x`; playable state is indicated by the blob, not button strokes.
- There is no board geometry fallback path in active gameplay. If the board is mounted, row, column, cell, and blob rectangles come from the same fixed coordinate system; if the game is not active, the board controller resets. This removes CSS `calc()`, DOMRect measurement drift, old class selectors, and viewport-dependent row/column mismatch from the signature blob.
