...
- Board animation uses CSS-only progressive reveal groups from `-9` through `+9`, a single filled selection blob exactly behind the playable track, and a selected-cell compression fade. Each turn transition morphs the same blob from the previous row/column, into the selected card, pauses while the card is removed, then grows into the next legal row/column. Column growth uses a longer settle duration so its perceived travel speed matches horizontal movement. Input stays locked until the full sequenced transition settles, so the opponent can only choose after the blob reaches their playable track. Claimed cells visually disappear like removed tiles, never show as empty buttons, and are never marked with `x`; playable state is indicated by the blob, not button strokes.
