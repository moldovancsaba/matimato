...
- Board animation uses CSS-only progressive reveal groups from `-9` through `+9`, a single filled selection blob exactly behind the playable track, and an explicit turn state machine: `collapse -> hold -> grow -> ready`. The accepted move updates the real board immediately, so the selected card is removed from the grid state at once. The blob is the action layer: it collapses the current row/column ribbon into the selected cell position, holds briefly as the in phase completes, then grows into the next legal row/column. The first move has no prior ribbon, so the blob starts at the selected cell position before growing into the first constrained track. Collapse and grow phases use equal linear time slices, with a short middle hold. Column transitions use a longer settle duration so perceived vertical travel speed matches horizontal movement. Input stays locked until the full sequenced transition settles, so the opponent can only choose after the blob reaches their playable track; non-match, unconstrained, and terminal states explicitly clear the lock so the board cannot become permanently inactive. Claimed cells visually disappear like removed tiles, never show as empty buttons, and are never marked with `x`; playable state is indicated by the blob, not button strokes.
